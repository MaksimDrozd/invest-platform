...
import type { UserFundBalance, WithdrawalAddress } from '../types';

interface WithdrawalFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onProceed: (data: { selectedFund: UserFundBalance; address: WithdrawalAddress; amount: number }) => void;
  onBack: () => void;
  selectedFund: UserFundBalance;
}
...
export default function WithdrawalFormModal({ 
  ...
  const availableAmount = selectedFund.availableForWithdrawal;
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="text-sm text-gray-600">
              Available to withdraw: <span className="font-medium">{availableAmount.toLocaleString()} {selectedFund.currency}</span>
            </div>
          </div>
          ...
          {amount && !isValidAmount && (
            <p className="text-xs text-red-600 mt-2">
              {currentAmount > availableAmount 
                ? `Amount exceeds available balance of ${availableAmount} ${selectedFund.currency}`
